...
class Node:
    ...
    def update_bounds_below(self):
        ...
        if self.feature in self.left_child.lower:
            current_lower = self.left_child.lower[self.feature]
        ...
        if self.feature in self.right_child.upper:
            current_upper = self.right_child.upper[self.feature]
...
class Decision_Tree:
    ...
    def fit(self, explanatory, target, verbose=0):
        """Train the decision tree using explanatory data and targets."""
        ...
        if verbose == 1:
            ...
            leaf_count = self.count_nodes(only_leaves=True)
    ...
    def random_split_criterion(self, node):
        ...
        while diff == 0:
            ...
            feature_values = self.explanatory[:, feature][
                node.sub_population
            ]
    ...
    def fit_node(self, node):
        """Recursively split a node and build its child nodes."""
    ...
    def get_leaf_child(self, node, sub_population):
        """Create a leaf child for the supplied sub-population."""
        value = np.argmax(
            np.bincount(self.target[sub_population])
        )
    ...
    def get_node_child(self, node, sub_population):
        """Create an internal child for the supplied sub-population."""
        child = Node()
